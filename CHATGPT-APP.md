...
ChatGPT creates the detailed itinerary and calls `render_travel_itinerary`. The MCP server validates the structured itinerary and returns an interactive x-Travel Guide widget. The server does **not** call the OpenAI API and does not need an OpenAI API key.
...
5. Start a new conversation, add x-Travel Guide from the **+ → More** menu, and say: `Plan a trip for me.`
