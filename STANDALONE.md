...
The `standalone-pages.yml` workflow packages only the three application files and deploys them to GitHub Pages. In the repository settings, set **Pages → Source** to **GitHub Actions**, then push to the `main` branch.
...
- The Content Security Policy permits connections only to Open-Meteo's forecast and geocoding endpoints.
